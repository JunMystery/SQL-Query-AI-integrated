...
class TestSchemaRAG(unittest.TestCase):
    ...
    def test_context_formatting(self) -> None:
        prompt = "Tính tổng số Tasks"
        context = SchemaRAG.get_rag_schema_context(prompt, self.tables, self.annotations, max_tables=1)
        self.assertIn("Bảng `habits` (Thói quen hoạt động): các cột [habit_id, user_id, title, from_date, to_date]", context)
        self.assertNotIn("Bảng `users`", context)
